...
Install [the relay nginx locations](../ops/runtime-relay.nginx.conf) inside the server block before exposing `/runtime/`. Disable edge/CDN caching, request capture, analytics payload capture, and error-body recording for that namespace and the device connection. The application sets no-store headers and never stores desktop payloads, but cannot change an external reverse proxy's logging policy. HTTPS/WSS is required for pairing; the server is a trusted plaintext relay, not an end-to-end encrypted transport.
...
`/api/logic/stream` without buffering for chat, theme, and layout updates.

[Cloudflare templates](../ops/cloudflare-relay.json) add two narrowly scoped rules:
cache bypass and content settings for `/runtime/` plus `/api/devices/connect`. Add these
