...
Rime file reads are paged by line, with a column cursor for long lines and separate access to conflicting disk text. Git tool reads can be scoped by path and remain within the tool response budget; the Changes ward keeps its full diff. Failed turns retain their completed actions in history, and shared model requests send whitespace heartbeats while reasoning to keep the relay connection active.
