...
Codex and Claude Code must already be installed and signed in locally. Setup links are in session settings. No CLI or provider credential is installed or copied automatically. **Allow Rime to type** is independent of CLI permissions and takes effect immediately; it includes answering native CLI prompts. Human input ownership survives idle time and reloads. **Take control** pauses agent input; **Let Rime type** releases human ownership. New sessions keep Rime input off unless the user enables it. Codex/Claude use **Standard** permission prompts by default; **Unrestricted** explicitly bypasses their protections on the next start. Shells run with the desktop account's OS permissions and do not expose a misleading CLI permission mode. **Start again** opens a new process with saved settings after the previous process ends, without replaying its task.
...
Rime file reads are paged by line, with a column cursor for long lines and separate access to conflicting disk text. Git tool diffs can be scoped by path and are capped; large status/search results still need the broader pagination work described in the [harness follow-up](rime-agent-harness-follow-up.md). The Changes ward keeps its full diff. Project edits return a compact saved/revision receipt instead of echoing the whole file. An oversized tool response is reported as omitted, without falsely reporting a failed operation or encouraging a repeated write. Failed turns retain their completed actions in history, and shared model requests send whitespace heartbeats while reasoning to keep the relay connection active.
